...
fun WebTesterScreen(onBack: () -> Unit) {
    ...
    fun fetch(url: String, label: String): String {
        return try {
            ...
            conn.disconnect()
            "[$label] HTTP $code\n${body.take(500)}\n"
        } catch (e: Exception) { "[$label] ERROR: ${e.message}\n" }
    }
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            HunterCard {
                ...
                OutlinedTextField(
                    ...
                    label = { Text("Domain (e.g. example.com)", fontFamily = FontFamily.Monospace, color = HunterTextDim) },
                ...
                ) { Text("[ SCAN ASSET LINKS ]", color = HunterBg, fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold) }
            }
            ...
            HunterCard {
                ...
                Text("QUICK PATH SCAN:", color = HunterTextDim, fontFamily = FontFamily.Monospace, fontSize = 11.sp)
                ...
                Button(
                    ...
                    shape    = RoundedCornerShape(4.dp), enabled = !running
                ) { Text("[ SCAN PATHS ]", color = HunterBg, fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold) }
            }
            ...
            if (output.isNotBlank()) {
                HunterCard {
                    Text("RESULT", color = HunterTextDim, fontFamily = FontFamily.Monospace, fontSize = 11.sp)
