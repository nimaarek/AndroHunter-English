...
fun AccessibilityMonitorScreen(onBack: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().background(HunterBg)) {
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            HunterCard {
                Text("Open Accessibility Settings to enable the service",
                ...
                ) { Text("[ ACCESSIBILITY SETTINGS ]", color = HunterBg, fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold) }
            }

            OutlinedTextField(
                value = filter, onValueChange = { filter = it },
                label = { Text("Filter by package", fontFamily = FontFamily.Monospace, color = HunterTextDim) },
