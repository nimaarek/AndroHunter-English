...
class PayloadEngine(private val context: Context) {
    ...
    suspend fun fireIntentPayload(
        ...
        return@withContext try {
            ...
            delay(500)
            PayloadResult(payload, "intent", ResultStatus.SAFE, "Intent sent")
        } catch (e: Exception) {
            PayloadResult(payload, "intent", ResultStatus.ERROR, e.message ?: "Error")
        }
    }
    ...
    suspend fun fireHttpPayload(
        ...
        return@withContext try {
            ...
            PayloadResult(payload, type.label, ResultStatus.ERROR, e.message ?: "Connection error")
        }
    }
}
