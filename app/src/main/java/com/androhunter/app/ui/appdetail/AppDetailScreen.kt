...
fun AppDetailScreen(
    ...
    Column(Modifier.fillMaxSize().background(HunterBg).verticalScroll(rememberScrollState())) {
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            HunterCard {
                Text("PACKAGE INFO", color = HunterTextDim, fontFamily = FontFamily.Monospace, fontSize = 11.sp)
                Spacer(Modifier.height(8.dp))
                InfoRow("Package",    packageName)
                InfoRow("Version",    packageInfo?.versionName ?: "?")
            ...
            // Action grid
            Text("TOOLS", color = HunterTextDim, fontFamily = FontFamily.Monospace, fontSize = 11.sp)
